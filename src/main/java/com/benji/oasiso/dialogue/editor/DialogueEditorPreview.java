...
public final class DialogueEditorPreview {
    ...
    private static final ResourceLocation DEFAULT_SPRITE = ResourceLocation.fromNamespaceAndPath(Oasiso.MODID, "textures/gui/dialogue/azumaal_sprite_idle.png");
    ...
    private static List<Glyph> layoutGlyphs(Font font, String text, int maxWidth, int lineHeight) {
        ...
        while (i < text.length()) {
            ...
            for (int j = i; j < end; j++) {
                ...
                int cw = font.width(String.valueOf(letter));
                if (x > 0 && x + cw > maxWidth) {
                    x = 0;
                    y += lineHeight;
                }
                result.add(new Glyph(j, letter, x, y));
                x += cw;
            }
            i = end;
        }
