...
public final class EntropyLiquidVfx {
    ...
    public static void tick(TickEvent.ClientTickEvent event) {
        ...
        if (world != mc.level) {
            ...
            EntropyLiquidParticle.resetBudget(world);
        }
            ...
            for (int i = 0; i < underwaterCount; i++) {
                for (int attempt = 0; attempt < 8; attempt++) {
                    ...
                    if (contains(world, x, y, z) && world.getBlockState(BlockPos.containing(x, y, z)).getCollisionShape(world, BlockPos.containing(x, y, z)).isEmpty()) {
                        world.addParticle(Oasiso.ENTROPY_BUBBLES.get(), x, y, z, 0, 0, 0);
                        break;
                    }
                }
            }
    ...
    private static void spawnSurface(boolean steam) {
        ...
        world.addParticle(steam ? Oasiso.ENTROPY_STEAM.get() : Oasiso.ENTROPY_BUBBLES.get(), s.pos.getX() + u, s.pos.getY() + h + .025, s.pos.getZ() + v, 0, steam ? 0 : .15 + world.random.nextDouble() * .45, 0);
    }
