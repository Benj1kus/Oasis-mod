...
public final class ChaosCameraShakeEvents {
    ...
    public static void onCameraAngles(
        ...
        float yawOffset =
                (
                        Mth.sin(time * 0.91F) * 0.7F
                                + Mth.sin(time * 1.73F) * 0.3F
                ) * intensity;

        float pitchOffset =
                (
                        Mth.sin(time * 1.19F) * 0.65F
                                + Mth.sin(time * 0.53F) * 0.35F
                ) * intensity;
        ...
        event.setYaw(
                event.getYaw() + yawOffset
        );

        event.setPitch(
                event.getPitch() + pitchOffset
        );

        event.setRoll(
                event.getRoll() + rollOffset
        );
    }
}