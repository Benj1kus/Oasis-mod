...
public class AzumaalShockwaveLayer extends GeoRenderLayer<AzumaalEntity> {
    ...
    public AzumaalShockwaveLayer(GeoRenderer<AzumaalEntity> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack poseStack, AzumaalEntity entity, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        long start = entity.getClientPressureShockwaveStartNanos();
        if (start <= 0L) return;
