...
public class SwordHeartAuraLayer extends GeoRenderLayer<SwordHeartEntity> {
    ...
    public void render(PoseStack poseStack, SwordHeartEntity entity, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        double innerExpand = 0.10D + pulse * 0.03D;

        drawEnergyBox(consumer, matrix, -halfWidth - innerExpand, -0.03D, -halfWidth - innerExpand, halfWidth + innerExpand, height + 0.03D, halfWidth + innerExpand, 40, 255, 215, 42 + (int) (pulse * 36.0F));

        float secondPulse = 0.5F + 0.5F * Mth.sin(time * 0.18F + 2.1F);

        double outerExpand = 0.19D + secondPulse * 0.045D;

        drawEnergyBox(consumer, matrix, -halfWidth - outerExpand, -0.06D, -halfWidth - outerExpand, halfWidth + outerExpand, height + 0.06D, halfWidth + outerExpand, 70, 225, 255, 20 + (int) (secondPulse * 24.0F));
