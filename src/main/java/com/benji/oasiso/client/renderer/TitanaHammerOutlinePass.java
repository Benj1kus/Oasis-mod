...
final class TitanaHammerOutlinePass {
    private static int framebuffer, colorTexture, depthTexture, width, height;
    private static int modelVao, modelVbo, screenVao, screenVbo;

    private TitanaHammerOutlinePass() {
    }

    static void draw(List<float[]> vertices, int texture, Matrix4f inverse, float minX, float minY, float maxX, float maxY, boolean crossesCamera, float alpha) {
        State old = new State();
        try (MemoryStack memory = MemoryStack.stackPush()) {
            ...
            int mask = TitanaHammerShaders.mask.getId();
            ...
            matrix(mask, "ModelInverse", inverse, memory);
            GL20.glUniform2f(GL20.glGetUniformLocation(mask, "FadeRange"), TitanaHammerRenderer.FADE_START / 16F, TitanaHammerRenderer.FADE_END / 16F);
            ...
            float radius = Math.min(6F, TitanaHammerRenderer.OUTLINE_WIDTH * Math.max(.65F, h / 1080F));
            if (!scissor(old, minX, minY, maxX, maxY, crossesCamera, radius)) return;
            GL11.glDepthMask(false);
            GL11.glEnable(GL11.GL_BLEND);
            GL20.glBlendEquationSeparate(GL14.GL_FUNC_ADD, GL14.GL_FUNC_ADD);
            GL14.glBlendFuncSeparate(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA, GL11.GL_ONE, GL11.GL_ONE_MINUS_SRC_ALPHA);
            int outline = TitanaHammerShaders.outline.getId();
            ...
            GL20.glUniform2f(GL20.glGetUniformLocation(outline, "TexelSize"), 1F / w, 1F / h);
            GL20.glUniform1f(GL20.glGetUniformLocation(outline, "Radius"), radius);
            GL20.glUniform1f(GL20.glGetUniformLocation(outline, "Opacity"), TitanaHammerRenderer.OPACITY * alpha);
            GL13.glActiveTexture(GL13.GL_TEXTURE0);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, colorTexture);
            GL13.glActiveTexture(GL13.GL_TEXTURE1);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, depthTexture);
            bindMesh(screenVao, screenVbo, outline, memory.floats(-1, -1, 0, 0, 0, 1, -1, 0, 1, 0, 1, 1, 0, 1, 1, -1, -1, 0, 0, 0, 1, 1, 0, 1, 1, -1, 1, 0, 0, 1));
    ...
    private static boolean scissor(State s, float minX, float minY, float maxX, float maxY, boolean crossesCamera, float radius) {
        int vx = s.viewport[0], vy = s.viewport[1], w = s.viewport[2], h = s.viewport[3];
        ...
        if (s.scissor) {
            x0 = Math.max(x0, s.box[0]);
            y0 = Math.max(y0, s.box[1]);
            x1 = Math.min(x1, s.box[0] + s.box[2]);
            y1 = Math.min(y1, s.box[1] + s.box[3]);
        }
    ...
    private static void ensure(int w, int h) {
        ...
        colorTexture = texture(GL11.GL_RGBA8, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE);
        depthTexture = texture(GL14.GL_DEPTH_COMPONENT24, GL11.GL_DEPTH_COMPONENT, GL11.GL_UNSIGNED_INT);
        GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, GL11.GL_TEXTURE_2D, colorTexture, 0);
        GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_DEPTH_ATTACHMENT, GL11.GL_TEXTURE_2D, depthTexture, 0);
        GL11.glDrawBuffer(GL30.GL_COLOR_ATTACHMENT0);
        if (GL30.glCheckFramebufferStatus(GL30.GL_FRAMEBUFFER) != GL30.GL_FRAMEBUFFER_COMPLETE)
            throw new IllegalStateException("Titana hammer outline framebuffer is incomplete");
    }

    private static int texture(int internal, int format, int type) {
    ...
    private static void deleteTarget() {
        ...
        framebuffer = colorTexture = depthTexture = 0;
    }
