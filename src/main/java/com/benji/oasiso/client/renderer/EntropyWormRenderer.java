...
import com.benji.oasiso.client.model.EntropyWormModel;
...
public class EntropyWormRenderer extends GeoEntityRenderer<EntropyWormEntity> {
    ...
    public EntropyWormRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new EntropyWormModel());
        this.shadowRadius = 0.4f;
        addRenderLayer(new GlowmaskLayer<>(this));
    }

    @Override
    public void render(EntropyWormEntity entity, float yaw, float partial, PoseStack pose,
        ...
        if (mesh.isEmpty() || !EntropyWormShaders.ready()) return;
        ...
        EntropyWormOutlinePass.draw(mesh, texture, minX, minY, maxX, maxY, crossesCamera, time, OUTLINE_OPACITY);
        mesh.clear();
    }

    @Override
    public void actuallyRender(PoseStack pose, EntropyWormEntity entity, BakedGeoModel model,
        ...
        capture = !reRender && !entity.isInvisible() && EntropyWormShaders.ready()
    ...
    @Override
    public boolean shouldRender(EntropyWormEntity entity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }
}
