...
import com.benji.oasiso.client.model.EntropyCreatureModel;
...
public class EntropyCreatureRenderer extends GeoEntityRenderer<EntropyCreatureEntity> {
    ...
    public EntropyCreatureRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new EntropyCreatureModel());
        this.shadowRadius = 0.4f;
        addRenderLayer(new GlowmaskLayer<>(this));
    }

    @Override
    public void render(EntropyCreatureEntity entity, float yaw, float partial, PoseStack pose,
        ...
        if (mesh.isEmpty() || !EntropyCreatureShaders.ready()) return;
        ...
        EntropyCreatureOutlinePass.draw(mesh, texture, minX, minY, maxX, maxY, crossesCamera, time, OUTLINE_OPACITY);
        mesh.clear();
    }

    @Override
    public void actuallyRender(PoseStack pose, EntropyCreatureEntity entity, BakedGeoModel model,
        ...
        capture = !reRender && !entity.isInvisible() && EntropyCreatureShaders.ready()
    ...
    @Override
    public boolean shouldRender(EntropyCreatureEntity entity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }
}
