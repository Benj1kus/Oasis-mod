...
public class EntropySpiderRenderer extends GeoEntityRenderer<EntropySpiderEntity> {
    ...
    @Override
    public void render(EntropySpiderEntity entity, float yaw, float partial, PoseStack pose,
        ...
        if (mesh.isEmpty() || !EntropySpiderShaders.ready()) return;
        ...
        EntropySpiderOutlinePass.draw(mesh, texture, minX, minY, maxX, maxY, crossesCamera, time, OUTLINE_OPACITY);
        mesh.clear();
    }

    @Override
    public void actuallyRender(PoseStack pose, EntropySpiderEntity entity, BakedGeoModel model,
        ...
        capture = !reRender && !entity.isInvisible() && EntropySpiderShaders.ready()
