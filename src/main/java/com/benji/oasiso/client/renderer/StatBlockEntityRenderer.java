...
public class StatBlockEntityRenderer implements BlockEntityRenderer<StatBlockEntity> {
    ...
    public void render(StatBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        Minecraft.getInstance().getItemRenderer().renderStatic(itemStack, ItemDisplayContext.FIXED, glowLight, packedOverlay, poseStack, bufferSource, blockEntity.getLevel(), 0);
    ...
    private GlowColor getGlowColor(ItemStack stack) {
        ...
        // LEGENDARY
        if (stack.is(Items.GOLDEN_APPLE) || stack.is(Items.ENCHANTED_GOLDEN_APPLE) || stack.is(Items.GOLDEN_CARROT) || stack.is(Items.TOTEM_OF_UNDYING) || stack.is(Items.END_CRYSTAL) || stack.is(Items.ELYTRA) || stack.is(Oasiso.SUPER_GOLD_HELMET.get()) || stack.is(Oasiso.SUPER_GOLD_BOOTS.get()) || stack.is(Oasiso.SUPER_GOLD_CHESTPLATE.get()) || stack.is(Oasiso.SUPER_GOLD_LEGGINGS.get()) || stack.is(Oasiso.TITANA_HAMMER.get()) || stack.is(Oasiso.KARAKOLIT_INGOT.get()) || stack.is(Oasiso.NEPHRITIS_CORE.get()) || stack.is(Items.TRIDENT) || path.contains("netherite")) {
            return new GlowColor(230, 50, 230, 100, 30, 255);
        }
        // GOLD
        if (path.contains("golden_") || path.startsWith("gold_")) {
            if (path.contains("sword") || path.contains("pickaxe") || path.contains("axe") || path.contains("shovel") || path.contains("hoe") || path.contains("helmet") || path.contains("chestplate") || path.contains("leggings") || path.contains("boots") || path.contains("horse_armor")) {
