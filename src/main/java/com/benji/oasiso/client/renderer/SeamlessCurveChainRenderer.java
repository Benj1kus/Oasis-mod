...
public class SeamlessCurveChainRenderer implements BlockEntityRenderer<SeamlessCurveChainBlockEntity> {

    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Oasiso.MODID, "textures/block/seemless_curve_chain.png");
    ...
    public SeamlessCurveChainRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(SeamlessCurveChainBlockEntity chain, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        if (!chain.isConnected() || chain.getLevel() == null) {
        ...
        double bounce = calculatePlacementBounce(chain, partialTick);
        sag *= 1.0D + bounce * BOUNCE_STRENGTH;
        double width = BANNER_WIDTH * (1.0D - bounce * 0.025D);
        Vec3 renderOrigin = new Vec3(chain.getBlockPos().getX(), chain.getBlockPos().getY(), chain.getBlockPos().getZ());

        renderRibbon(chain.getLevel(), poseStack, bufferSource, NORMAL_RENDER_TYPE, start, end, startFace, renderOrigin, sag, width, 255, 255, 255, 255, packedOverlay, false);
    }
    ...
    private static double calculatePlacementBounce(SeamlessCurveChainBlockEntity chain, float partialTick) {
        if (chain.getLevel() == null || chain.getConnectionGameTime() < 0L) {

            return 0.0D;
        }

        double age = chain.getLevel().getGameTime() + partialTick - chain.getConnectionGameTime();
