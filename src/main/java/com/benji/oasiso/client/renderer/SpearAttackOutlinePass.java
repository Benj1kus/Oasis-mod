...
final class SpearAttackOutlinePass {
    ...
    private SpearAttackOutlinePass() {
    }
    ...
    static void draw(List<float[]> vertices, int texture, float minX, float minY, float maxX, float maxY, boolean crossesCamera, float time, float alpha) {
        ...
        try (MemoryStack memory = MemoryStack.stackPush()) {
            ...
            int mask = SpearAttackShaders.mask.getId();
            ...
            int horizontal = SpearAttackShaders.distance.getId();
            ...
            int outline = SpearAttackShaders.outline.getId();
