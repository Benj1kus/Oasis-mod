...
public class LieBlockRenderer implements BlockEntityRenderer<LieBlockEntity> {
    ...
    public void render(LieBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        if (player != null) {
            ...
            for (ItemStack armor : player.getArmorSlots()) {
                if (armor.is(Oasiso.SUPER_GOLD_HELMET.get()) || armor.is(Oasiso.SUPER_GOLD_CHESTPLATE.get()) || armor.is(Oasiso.SUPER_GOLD_LEGGINGS.get()) || armor.is(Oasiso.SUPER_GOLD_BOOTS.get())) {
                    armorCount++;
                }
            }
        ...
        if (be.isPhasing() && !hasArmor) {
            if (player != null) {
                BlockPos fadeOrigin = be.getFadeOriginPos();

                double distance = Math.sqrt(player.distanceToSqr(fadeOrigin.getX() + 0.5D, fadeOrigin.getY() + 0.5D, fadeOrigin.getZ() + 0.5D));

                double multiplier = be.hasNephritisSource() ? LieBlock.NEPHRITIS_RANGE_MULTIPLIER : 1.0D;

                double invisibleDistance = LieBlock.NORMAL_INVISIBLE_DISTANCE * multiplier;

                double fadeDistance = LieBlock.NORMAL_FADE_DISTANCE * multiplier;

                alpha = (float) Mth.clamp((distance - invisibleDistance) / fadeDistance, 0.0D, 1.0D);
            }
        }
    ...
    private static class AlphaWrapper implements VertexConsumer {
        ...
        public VertexConsumer color(int r, int g, int b, int a) {
            delegate.color(r, g, b, (a * this.alpha) / 255);
            return this;
        }
        ...
        public void defaultColor(int r, int g, int b, int a) {
            delegate.defaultColor(r, g, b, (a * this.alpha) / 255);
        }
