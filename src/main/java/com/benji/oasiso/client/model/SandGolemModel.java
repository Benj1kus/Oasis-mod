...
public class SandGolemModel extends GeoModel<SandGolemEntity> {
    ...
    public ResourceLocation getAnimationResource(SandGolemEntity animatable) {
        ...
        if (stage <= 2)
            return ResourceLocation.fromNamespaceAndPath(Oasiso.MODID, "animations/sand_golem.animation.json");

        if (stage == 3)
            return ResourceLocation.fromNamespaceAndPath(Oasiso.MODID, "animations/sand_golem_stage3.animation.json");
