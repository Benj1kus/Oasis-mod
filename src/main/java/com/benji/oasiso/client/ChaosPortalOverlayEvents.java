...
public final class ChaosPortalOverlayEvents {
    ...
    private static final ResourceLocation PORTAL_TEXTURE =
            ResourceLocation.fromNamespaceAndPath(
                    Oasiso.MODID,
                    "textures/block/chaos_portal.png"
            );
    ...
    private static void renderOverlay(
        ...
        int frame =
                (int) (
                        minecraft.level.getGameTime() / 2L
                                % FRAME_COUNT
                );

        float scaleX =
                guiWidth / (float) FRAME_SIZE;

        float scaleY =
                guiHeight / (float) FRAME_SIZE;
