...
public class EntropyBlock extends Block {
    ...
    private static final double ENTROPY_RADIUS_SQR = ENTROPY_RADIUS * ENTROPY_RADIUS;
    ...
    public void onPlace(BlockState state, Level level, BlockPos pos, BlockState oldState, boolean movedByPiston) {
        ...
        if (!level.isClientSide && !state.is(oldState.getBlock())) {
            ...
            if (level instanceof ServerLevel serverLevel) {
                trySpawnKrombul(serverLevel, pos, serverLevel.random);
            }
        }
    }

    private void trySpawnKrombul(ServerLevel level, BlockPos sourcePos, RandomSource random) {
        AABB spawnArea = new AABB(sourcePos).inflate(KROMBUL_SPAWN_PLAYER_RADIUS);
        ...
        int nearbyKrombulCount = level.getEntitiesOfClass(KrombulEntity.class, spawnArea, krombul -> krombul.isAlive() && krombul.distanceToSqr(Vec3.atCenterOf(sourcePos)) <= radiusSqr).size();
        ...
        KrombulEntity krombul = Oasiso.KROMBUL.get().spawn(level, sourcePos.above(2), MobSpawnType.MOB_SUMMONED);
        ...
        level.sendParticles(Oasiso.PURPLE_STARS.get(), krombul.getX(), krombul.getY() + krombul.getBbHeight() * 0.5D, krombul.getZ(), 35, 0.5D, 0.7D, 0.5D, 0.08D);
    }
        ...
        if (state.getBlock() != newState.getBlock() && !level.isClientSide) {
            for (Direction direction : Direction.values()) {
                ...
                if (level.getBlockState(neighbourPos).is(Oasiso.ENTROPY_VEIN.get())) {
                    level.scheduleTick(neighbourPos, Oasiso.ENTROPY_VEIN.get(), 2 + level.random.nextInt(4));
                }
            }
        }
    ...
    private void applyEntropyAura(ServerLevel level, BlockPos sourcePos) {
        ...
        for (Entity entity : entities) {
            ...
            if (entity instanceof LivingEntity livingEntity) {
                boolean protectedByArmor = livingEntity instanceof Player player && hasFullSuperGoldArmor(player);

                if (!protectedByArmor) {
                    livingEntity.addEffect(new MobEffectInstance(Oasiso.ENTROPY_EFFECT.get(), 12, 0, false, false, true));
                }
            }
            ...
            entity.addDeltaMovement(new Vec3(pull.x, pull.y * 0.35D, pull.z));
        }
    }
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        ...
        double y = pos.getY() + 0.9D + random.nextDouble() * 0.35D;
        double z = pos.getZ() + 0.15D + random.nextDouble() * 0.7D;

        double velocityX = (random.nextDouble() - 0.5D) * 0.01D;
        double velocityY = 0.005D + random.nextDouble() * 0.01D;
        double velocityZ = (random.nextDouble() - 0.5D) * 0.01D;

        level.addParticle(Oasiso.PURPLE_STARS.get(), x, y, z, velocityX, velocityY, velocityZ);
    }
    ...
    private void trySpread(ServerLevel level, BlockPos sourcePos, RandomSource random) {
        ...
        for (int attempt = 0; attempt < 12; attempt++) {
            ...
            if (canPlaceVein(level, target)) {
                level.setBlock(target, Oasiso.ENTROPY_VEIN.get().defaultBlockState().setValue(EntropyVeinBlock.FACING, dir).setValue(EntropyVeinBlock.STAGE, 0), 3);
                return;
            }

            BlockState targetState = level.getBlockState(target);
            if (targetState.is(Oasiso.ENTROPY_VEIN.get()) && targetState.getValue(EntropyVeinBlock.STAGE) == 0 && random.nextInt(3) == 0) {
    ...
    private List<BlockPos> collectFrontier(ServerLevel level, BlockPos sourcePos) {
        ...
        for (BlockPos scanPos : BlockPos.betweenClosed(min, max)) {
            ...
            if (scanState.is(Oasiso.ENTROPY_VEIN.get()) && scanState.getValue(EntropyVeinBlock.STAGE) == 1) {
