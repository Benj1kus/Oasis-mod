...
public class LieBlockEntity extends BlockEntity {
    ...
    public BlockPos getFadeOriginPos() {
        return this.nephritisSourcePos != null
                ? this.nephritisSourcePos
                : this.worldPosition;
    }

    public void considerNephritisSource(BlockPos sourcePos) {
        if (distanceSqr(sourcePos, this.worldPosition)
                > NephritisBlock.MIMIC_EFFECT_RADIUS
                * NephritisBlock.MIMIC_EFFECT_RADIUS) {
            return;
        }

        if (this.nephritisSourcePos == null
                || distanceSqr(sourcePos, this.worldPosition)
                < distanceSqr(
                this.nephritisSourcePos,
                this.worldPosition
        )) {
    ...
    private void setNephritisSource(
            @Nullable BlockPos sourcePos
    ) {
        BlockPos immutableSource =
                sourcePos == null
                        ? null
                        : sourcePos.immutable();
    ...
    private void refreshNephritisSource(
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    mutablePos.set(
                            this.worldPosition.getX() + x,
                            this.worldPosition.getY() + y,
                            this.worldPosition.getZ() + z
                    );

                    if (ignoredPos != null
                            && mutablePos.equals(ignoredPos)) {
                        continue;
                    }

                    if (!serverLevel.getBlockState(mutablePos)
                            .is(Oasiso.NEPHRITIS_BLOCK.get())) {
                        continue;
                    }
    ...
    private static double distanceSqr(
            BlockPos first,
            BlockPos second
    ) {
    ...
    protected void saveAdditional(CompoundTag tag) {
        ...
        if (this.nephritisSourcePos != null) {
            tag.putLong(
                    "NephritisSource",
                    this.nephritisSourcePos.asLong()
            );
        }
    }
    ...
    public void load(CompoundTag tag) {
        ...
        if (tag.contains(
            ...
            this.nephritisSourcePos =
                    BlockPos.of(tag.getLong("NephritisSource"));
        } else {
