...
public class SandGolemEntity extends Monster implements GeoEntity, GlowmaskEntity {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            ...
            if (this.lastStage != -1 && currentStage > this.lastStage) {
                ((ServerLevel) this.level()).sendParticles(new BlockParticleOption(ParticleTypes.FALLING_DUST, Blocks.SAND.defaultBlockState()), this.getX(), this.getY() + 1.5D, this.getZ(), 60, 0.6D, 1.0D, 0.6D, 0.1D);
    ...
    public void performAoEAttack() {
        ...
        if (!this.level().isClientSide) {
            ((ServerLevel) this.level()).sendParticles(new BlockParticleOption(ParticleTypes.BLOCK, Blocks.SAND.defaultBlockState()), this.getX(), this.getY() + 0.1D, this.getZ(), 150, 3.0D, 0.2D, 3.0D, 0.15D);
        }
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(new AnimationController<>(this, "action_controller", 0, event -> PlayState.CONTINUE).triggerableAnim("attack", RawAnimation.begin().thenPlay("attack")));
    }
    ...
    protected SoundEvent getAmbientSound() {
        ...
        SoundEvent[] sounds = {ModSounds.SANDGOLEM1.get(), ModSounds.SANDGOLEM2.get(), ModSounds.SANDGOLEM_STEP.get()};
