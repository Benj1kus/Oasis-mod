...
public class MonkiMergeGoal extends Goal {
    ...
    public boolean canUse() {
        if (this.mob.isMerging || this.mob.getHealth() > this.mob.getMaxHealth() / 2.0F || this.mob.mergeCooldown > 0)
            return false;

        List<MonkiEntity> list = this.mob.level().getEntitiesOfClass(MonkiEntity.class, this.mob.getBoundingBox().inflate(12.0D), e -> e != this.mob && !e.isMerging && e.mergeCooldown <= 0 && e.getHealth() <= e.getMaxHealth() / 2.0F);
    ...
    public boolean canContinueToUse() {
        return this.ally1 != null && this.ally1.isAlive() && this.ally2 != null && this.ally2.isAlive() && this.mergeTimer < 40;
    }
        ...
        if (this.mergeTimer >= 40 && !this.mob.level().isClientSide) {
            ...
            if (big != null) {
                ...
                if (this.mob.level() instanceof ServerLevel serverLevel) {
                    serverLevel.sendParticles(new BlockParticleOption(ParticleTypes.BLOCK, Blocks.SAND.defaultBlockState()), this.mergePos.x, this.mergePos.y + 1.0, this.mergePos.z, 150, 1.0, 1.5, 1.0, 0.15);
                }
