...
public class DasherMagneticGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        return this.target != null && this.mob.isMagnetic() && this.mob.magneticCooldown <= 0 && this.mob.distanceToSqr(this.target) <= 25.0D && this.mob.getAnimState() == 0;
    }
    ...
    private void createShockwave() {
        if (this.mob.level() instanceof ServerLevel sl) {
            this.mob.playSound(ModSounds.MAGNETIC.get(), 1.0F, 1.0F);
            ...
            for (int i = 0; i < 360; i += 10) {
                ...
                sl.sendParticles(new BlockParticleOption(ParticleTypes.FALLING_DUST, Blocks.SAND.defaultBlockState()), px + dx, py, pz + dz, 2, dx * 0.1, 0, dz * 0.1, 0.05);
            }
