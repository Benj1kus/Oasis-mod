...
public class BombulEntity extends Monster implements GeoEntity, GlowmaskEntity {
    ...
    private static final EntityDataAccessor<Integer> STATE = SynchedEntityData.defineId(BombulEntity.class, EntityDataSerializers.INT);
    ...
    public BombulEntity(EntityType<? extends Monster> type, Level level) {
        super(type, level);
    }
    ...
    public boolean isAwake() {
        return this.getAnimState() == STATE_IDLE || this.getAnimState() == STATE_SUPPORT;
    }
    ...
    public void tick() {
        ...
        if (this.tickCount % 10 == 0) {
            buffedSomething = BombulBuffHandler.applyBuffAround(this);
        }
        ...
        if (state == STATE_SUPPORT) {
            ...
            if (this.stateTimer <= 0) {
                this.setAnimState(STATE_IDLE);
                this.supportCooldown = SUPPORT_COOLDOWN_TIME;
            }

            return;
        }

        if (state == STATE_IDLE && buffedSomething && this.supportCooldown <= 0) {

            this.setAnimState(STATE_SUPPORT);
            this.stateTimer = SUPPORT_ANIMATION_TIME;

            this.playSound(ModSounds.SUPPORT.get(), 1.0F, 1.0F);
        }
    }

    private boolean hasWakeUpPlayer() {
        return !this.level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(WAKE_RADIUS), player -> player.isAlive() && !player.isCreative() && !player.isSpectator()).isEmpty();
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (heldStack.is(Oasiso.BOMBUL_BOTTLE_EMPTY.get())) {
            if (!this.level().isClientSide) {
                ...
                if (this.getAnimState() != STATE_SLEEP || this.forcedSleepTicks <= 0) {
                    return InteractionResult.CONSUME;
                }

                spawnGoldenBurst(45);

                player.setItemInHand(hand, new ItemStack(Oasiso.BOMBUL_BOTTLE.get()));
    ...
    public void spawnGoldenBurst(int particleCount) {
        ...
        serverLevel.sendParticles(Oasiso.GOLDEN_STARS.get(),

                this.getX(), this.getY() + this.getBbHeight() * 0.5D, this.getZ(),

                particleCount,

                this.getBbWidth() * 0.65D, this.getBbHeight() * 0.45D, this.getBbWidth() * 0.65D,
    ...
    public void die(DamageSource source) {
        ...
        if (!this.level().isClientSide && !this.explodedOnDeath) {

            this.explodedOnDeath = true;

            this.level().explode(this, this.getX(), this.getY() + this.getBbHeight() * 0.5D, this.getZ(), 4.0F, false, Level.ExplosionInteraction.NONE);
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putInt("BombulSupportCooldown", this.supportCooldown);
        tag.putInt("BombulForcedSleepTicks", this.forcedSleepTicks);
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.setAnimState(tag.getInt("BombulState"));
        this.forcedSleepTicks = tag.getInt("BombulForcedSleepTicks");

        this.stateTimer = tag.getInt("BombulStateTimer");

        this.supportCooldown = tag.getInt("BombulSupportCooldown");
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "controller", 0, event -> {
            ...
            if (state == STATE_WAKE_UP) {
                return event.setAndContinue(RawAnimation.begin().thenPlayAndHold("wake_up"));
            }

            if (state == STATE_SUPPORT) {
                return event.setAndContinue(RawAnimation.begin().thenPlayAndHold("support"));
            }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.BOMBUL_IDLE.get();
    }

    @Override
    public ResourceLocation getGlowmaskTexture() {
        return ResourceLocation.fromNamespaceAndPath(Oasiso.MODID, "textures/entity/emissive/bombul_emissive.png");
    }
