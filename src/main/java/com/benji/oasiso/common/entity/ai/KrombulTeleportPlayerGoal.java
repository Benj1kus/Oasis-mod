...
public class KrombulTeleportPlayerGoal extends Goal {
    ...
    public KrombulTeleportPlayerGoal(
    ...
    public boolean canUse() {
        if (this.krombul.getTarget() != null
                || this.krombul.isTeleporting()) {
            resetProximityTracking();
            return false;
        }

        if (this.krombul.tickCount
                < this.nextTeleportAllowedTick) {
            resetProximityTracking();
            return false;
        }
        ...
        if (!playerId.equals(this.trackedPlayerId)) {
            ...
            this.proximityStartTick =
                    this.krombul.tickCount;
            this.candidatePlayer =
                    nearestPlayer;
        ...
        return this.krombul.tickCount
                - this.proximityStartTick
                >= REQUIRED_NEARBY_TIME;
    }
    ...
    private void tickApproach() {
        ...
        this.krombul.getLookControl().setLookAt(
                player,
                30.0F,
                30.0F
        );
        ...
        double wantedY =
                this.krombul.findHoverY(
                        player.getX(),
                        player.getZ()
                );

        this.krombul.getNavigation().moveTo(
                player.getX(),
                wantedY,
                player.getZ(),
                1.15D
    ...
    private void beginTeleportAnimation() {
        ...
        if (this.krombul.level()
                instanceof ServerLevel serverLevel) {
            playRandomTeleportSound(
                    serverLevel,
                    this.krombul.getX(),
                    this.krombul.getY()
                            + this.krombul.getBbHeight() * 0.5D,
                    this.krombul.getZ()
            );
        }
    }
    ...
    private void performTeleport() {
        ...
        Vec3 destination =
                findSafeTeleportPosition(level, player);
        ...
        double angle =
                this.krombul.getRandom().nextDouble()
                        * Math.PI
                        * 2.0D;
    ...
    private void playRandomTeleportSound(
        ...
        SoundEvent selectedSound =
                sounds[this.krombul.getRandom()
                        .nextInt(sounds.length)];
    ...
    private Vec3 findSafeTeleportPosition(
        ...
        for (int attempt = 0;
            ...
            /*
             * Минимум 8 блоков, максимум 50.
             * sqrt даёт более равномерное распределение
    ...
    private boolean isSafeForPlayer(
            ServerLevel level,
            BlockPos feetPos
    ) {
        if (feetPos.getY()
        ...
        if (!feetState.getCollisionShape(
                level,
                feetPos
        ).isEmpty()) {
            return false;
        ...
        if (!headState.getCollisionShape(
                level,
                headPos
        ).isEmpty()) {
            return false;
    ...
    private void spawnTeleportBurst(
            ServerLevel level,
            double x,
            double y,
            double z,
    ...
    private ServerPlayer findNearestPlayer() {
        ...
        List<ServerPlayer> nearbyPlayers =
                level.getEntitiesOfClass(
                        ServerPlayer.class,
                        this.krombul.getBoundingBox()
                                .inflate(DETECTION_RADIUS),
                        player ->
                                player.isAlive()
                                        && !player.isSpectator()
        ...
        for (ServerPlayer player : nearbyPlayers) {
            double distanceSqr =
                    player.distanceToSqr(this.krombul);
    ...
    private double horizontalDistanceSqr(
            ServerPlayer player
    ) {
        double x =
                player.getX()
