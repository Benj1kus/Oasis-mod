...
public final class OsirisRealmConfig {
    ...
    static {
        ...
        BUILDER.push("Osiris Damage Scaler");
        AZUMAAL_DAMAGE_SCALER_MAX_MULTIPLIER = BUILDER.comment("Maximum total multiplier that Osiris Damage Scaler may reach.", "2.0 = scaler may increase final damage up to 2x the base damage.", "The scaler formula itself is NOT changed by this config.").defineInRange("maxDamageMultiplier", 2.0D, 1.0D, 20.0D);
        BUILDER.pop();

        BUILDER.push("Osiris Attack Timing");
        ...
        AZUMAAL_POST_SPAWN_ATTACK_COOLDOWN = intValue("postSpawnAttackCooldown", "Cooldown before Osiris can attack after the intro/spawn phase, in ticks.", 40, 0, 12000);
        BUILDER.pop();

        BUILDER.push("Osiris Melee Attacks");
        ...
        AZUMAAL_PREPARE_BACK_DISTANCE = doubleValue("prepareBackDistance", "How far Osiris moves backwards before a melee attack.", 0.95D, 0.0D, 32.0D);
        ...
        AZUMAAL_MELEE_DAMAGE_MULTIPLIER = doubleValue("meleeDamageMultiplier", "Multiplier applied to Osiris ATTACK_DAMAGE for normal melee hits.", 1.0D, 0.0D, 20.0D);
        BUILDER.pop();

        BUILDER.push("Osiris Double Attack");
        AZUMAAL_DOUBLE_APPROACH_TICKS = intValue("approachTicks", "Ticks spent approaching before the double attack.", 5, 1, 200);
        AZUMAAL_DOUBLE_APPROACH_DISTANCE = doubleValue("approachDistance", "Distance left between Osiris and target after double-attack approach.", 2.0D, 0.0D, 32.0D);
        ...
        BUILDER.push("Osiris Throw Attack");
        AZUMAAL_THROW_APPROACH_TICKS = intValue("approachTicks", "Ticks spent approaching before the throw.", 5, 1, 200);
        AZUMAAL_THROW_APPROACH_DISTANCE = doubleValue("approachDistance", "Distance left between Osiris and target after throw approach.", 2.0D, 0.0D, 32.0D);
        ...
        AZUMAAL_AIR_CHASE_TICKS = intValue("airChaseTicks", "Ticks used to move Osiris into position above the airborne target.", 15, 1, 1200);
        AZUMAAL_AIR_HEIGHT_OFFSET = doubleValue("airHeightOffset", "Height offset Osiris keeps relative to the airborne target.", 1.75D, -16.0D, 32.0D);
        ...
        AZUMAAL_AIR_FOLLOW_FACTOR = doubleValue("airFollowFactor", "How quickly Osiris follows the target after the initial air chase.", 0.28D, 0.0D, 1.0D);
        BUILDER.pop();

        BUILDER.push("Osiris Summon Attacks");
        AZUMAAL_SUMMON_WEAK_ANIMATION_DURATION = intValue("weakAnimationDuration", "Tick when Osiris returns to idle animation during the weak summon attack.", 60, 1, 1200);
        ...
        AZUMAAL_MEGA_RANDOM_MIN_RADIUS = doubleValue("megaRandomMinRadius", "Minimum scattered-circle distance from Osiris.", 6.0D, 0.0D, 128.0D);
        AZUMAAL_MEGA_RANDOM_MAX_RADIUS = doubleValue("megaRandomMaxRadius", "Maximum scattered-circle distance from Osiris.", 20.0D, 0.0D, 128.0D);
        ...
        BUILDER.push("Osiris Eyes Attack");
        ...
        BUILDER.push("Osiris Clone Attack");
        AZUMAAL_CLONE_SUMMON_DURATION = intValue("summonDuration", "Delay before the clone formation begins, in ticks.", 60, 1, 1200);
        AZUMAAL_CLONE_COUNT = intValue("cloneCount", "Number of Osiris clones created by the rainbow clone attack.", 5, 1, 15);
        ...
        BUILDER.push("Osiris Defense Attack");
        ...
        BUILDER.push("Osiris Parkour Attack");
        AZUMAAL_PARKOUR_HEIGHT = doubleValue("height", "How high Osiris rises during the parkour attack.", 20.0D, 0.0D, 128.0D);
        ...
        BUILDER.comment("Each entry uses: entity_id|min|max", "Example: minecraft:zombie|2|5", "Optional fourth value is a random-choice group: entity_id|min|max|group_name", "Only ONE entry from each group is selected when a wave starts.", "This preserves the default INSECTS_BRUTAL behavior where Titana OR two Dashers are selected.", "Use 0|0 as min/max to effectively disable an entry. Invalid/non-Mob entity IDs are skipped safely.");
